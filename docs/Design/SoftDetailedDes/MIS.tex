...
vision, specifically OpenCV and MediaPipe, to analyze user video recording and perform analysis. 
...
\item[\texttt{const router = useRouter();}] 
    Expo Router instance for navigation after form submission. Imported from \texttt{expo-router}.
...
\item[\texttt{export const checkEmailExists}] 
    Async function constant that checks if an email is already registered in Firebase Authentication. Function accepts an email string as a parameter. Returns \texttt{true} if the email exists, \texttt{false} otherwise. Used to prevent duplicate account creation.

\item[\texttt{export const licenseFormatValid}] 
    Function constant that validates physiotherapist license number format (XX-123456). Accepts a license number string as a parameter. Returns an object containing \texttt{isValid} (boolean) and \texttt{errorMessage} (string or null).

\item[\texttt{export const registerPhysio}] 
    Async function constant that registers a new physiotherapist account. Type: \texttt{(params: \{name, email, password, licenseNumber, birthday\}) => Promise<\{uid: string, role: "physio"\}>}. Returns \texttt{\{uid, role\}} on success.

\item[\texttt{export const registerPatient}] 
    Async function constant that registers a new patient account. Type: \texttt{(params: \{name, email, password, inviteCode, birthday\}) => Promise<\{uid: string, role: "patient"\}>}. Returns \texttt{\{uid, role\}} on success.

\item[\texttt{export const login}] 
    Async function constant that authenticates a user. Type: \texttt{(email: string, password: string) => Promise<\{uid: string, ...userData\}>}. Returns user data from Firestore on success.

\item[\texttt{export const logout}] 
    Async function constant that signs out the current user. Type: \texttt{() => Promise<void>}.
...
\textbf{\textit{\underline{authService.ts}}}
\begin{description}[style=nextline, leftmargin=2.5cm]
...
% CreatePatient
%\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{CreatePatient():} Creates a new patient account using an invite code.} \\
%\hline
%\rowcolor{white}
...
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{CreatePatient:} Screen component for patient registration.} \\
\hline
\rowcolor{white}
\texttt{props: none} &
\texttt{JSX.Element} - Renders registration form with validation &
\begin{tabular}{@{}l@{}}
\texttt{Internal validations}\\ \texttt{\& services:}\\
• Password: min 6 \\chars + at least one\\ number\\
• Birthday: YYYY-MM\\-DD format validation\\
• Email: existence \\check via \\checkEmailExists()\\
• Invite code: validated \\by licenseFormatValid()\\
• Navigation: \\router.replace() on \\success
\end{tabular} \\
...
% CreatePhysio
%\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{CreatePhysio():} Creates a new physiotherapist account after license validation.} \\
%\hline
%\rowcolor{white}
%\texttt{params: \{name: string, email: string, password: string, licenseNumber: string, birthday: string\}} &
%\texttt{Promise<\{uid: string, role: "physio"\}>} - Returns user ID and role &
%\texttt{Error} - Invalid license number or Firebase Auth error \\
%\hline
...
\begin{tabular}{@{}l@{}}
\texttt{Validations performed:}\\
• Password: min 6 \\chars + at least \\one number\\
• Birthday: \\YYYY-MM-DD format\\
• Email: existence \\check via \\checkEmailExists()\\
• License format \\validation handled by \\registerPhysio()
\end{tabular} \\

\end{longtable}

\underline{\textbf{\textit{authService.ts}}} \\
...
\textit{Note: CreatePatient and registerPatient have an important distinction although they seem similar. 
...
\noindent \textbf{Exported Access Programs: labels and function definitions} \label{section: ACexpaccprog}\\
\texttt{e}: email \\
\texttt{n}: name \\
\texttt{pw}: password\\
\texttt{lno}: licenseNumber\\
\texttt{code}: inviteCode\\
\texttt{bday}: birthday\\
\texttt{p}: (n, e, pw, lno, bday)\\
\texttt{pt}: (n, e, pw, code, bday)\\
\texttt{cat}: createdAt \\
\texttt{pid}: physioId \\
\texttt{v}: verified \\
\texttt{u}: uid \\
...
See \ref{section: ACexpaccprog} under exported access programs: labels and function definitions\\
\texttt{first}: user 1\\
\texttt{second}: user 2\\
...
\noindent See \ref{section: ACexpaccprog}: exported access programs: labels and function definitions.\\
\newline
\textit{\underline{E(e)}}\\
\textbf{Pre-condition:}
  e $\in db.email$\\
...
\textbf{Post-condition:}
  Returns true if email already exists in database, false otherwise.\\
...
\hline
\rowcolor{white}
account\_create & \makecell[lt]{\textit{Name}\\\textit{Role}\\\textit{Birthday}\\\textit{License\_no/}\\\textit{Invite\_code}\\\textit{Email}\\\textit{Password}} & \makecell[lt]{user\_account\_p\\or\\user\_account\_pt} & \makecell[lt]{Input\_format\_invalid\\Required\_field\_empty\\Account\_already\_\\exists\\Incorrect\_creds\\Invite\_code\_expired} \\
\hline
...
\textit{db}: 'User Account'\\
\textit{new\_db}: updated 'User Account' \\
...
\begin{itemize}
  ...
  \item Exception: Input\_format\_invalid, Required\_field\_empty, Account\_already\_exists, Incorrect\_creds, Invite\_code\_expired 
\end{itemize}

\noindent \textit{set\_user\_info}: \\
NA
...
\textbf{Exported Constants}
\begin{itemize}
  \item \textit{is\_valid}: is\_valid 
\end{itemize}
...
\item[\texttt{role: "patient" | "physio";}] 
    User role designation. Determines which fields are applicable:
    \begin{itemize}
        \item \texttt{"patient"} - Receives care from a physiotherapist
        \item \texttt{"physio"} - Provides care as a physiotherapist
    \end{itemize}

\item[\texttt{birthday?: string;}] 
    User's date of birth. Optional field common to both roles.

\item[\texttt{licenseNumber?: string;}] 
    Professional license number. \textbf{Physio only:} Required for physiotherapists during verification.

\item[\texttt{verified?: boolean;}] 
    Account verification status. Optional flag common to both roles.

\item[\texttt{physioId?: string;}] 
    Assigned physiotherapist identifier. \textbf{Patient only:} References the physio they are seeing.

\item[\texttt{inviteCode?: string;}] 
    Registration invitation code. \textbf{Patient only:} Used during account creation.

\item[\texttt{createdAt?: any;}] 
    Account creation timestamp. Optional field for auditing.

\item[\texttt{updatedAt?: any;}] 
    Last update timestamp. Optional field for tracking modifications.

\item[\texttt{deleted?: boolean;}] 
    Soft delete flag. Optional field indicating if the account is marked for deletion.

\item[\texttt{deletedAt?: any;}] 
    Deletion timestamp. Optional field recording when soft delete occurred.
\end{description}

\begin{itemize}
    \item \textbf{Patient object (\texttt{user\_acc\_patient}):} 
        \texttt{\{uid, email, name, role: "patient", birthday?, verified?, physioId?, inviteCode?, createdAt?, updatedAt?, deleted?, deletedAt?\}}
    
    \item \textbf{Physio object (\texttt{user\_acc\_physio}):}
        \texttt{\{uid, email, name, role: "physio", birthday?, licenseNumber?, verified?, createdAt?, updatedAt?, deleted?, deletedAt?\}}
    
    \item \textbf{Note:} \texttt{physioId} and \texttt{inviteCode} are exclusive to patients; \texttt{licenseNumber} is exclusive to PTs.
\end{itemize}
%%%%%%%%%%%%\\\\\\\\\\\\\\\\
...
\hline
\rowcolor{white}
get\_userdb\_info$^2$ & \makecell[lt]{\textit{Name}\\\textit{birthday(optional)}} & \makecell[lt]{\textit{user\_acc\_p}\\\textit{user\_acc\_pt}} & \makecell[lt]{UserNotFoundError\\DownloadError} \\
\hline
...
\textit{name}: name \\
\textit{id}: acc\_id \\
\textit{patient}: user\_acc\_p ([\textit{id}, \textit{name}, \textit{p}: role, \textit{bday}: birthday, \textit{email},
\textit{pw}]) \\ 
\textit{physio}: user\_acc\_pt ([\textit{id}, \textit{name}, \textit{pt}: role, \textit{ptbday}: birthday, \textit{email},
\textit{pw}]) \\ 
\noindent \textit{D(x)}: 'PT\_account\_delete(x)' \\
\textit{db}: 'User Account' \\
\textit{name\_has\_id}: {db.Name} $\mapsto$ \{db.acc\_id\} \\
\textit{user\_info}: {db.acc\_id} $\mapsto$ \{db.Name, db.role, db.birthday, db.email\} \\

\noindent \textbf{State Invariant} \\
$\exists$ $acc\_id$ $|$ $(email \cap pw \notin \emptyset) $ \\
%$D(acc\_id) \implies patient \notin db $
...
\begin{itemize}
  ...
  \item No two users have the same invite codes.
  \item No two PTs have the same license numbers.
\end{itemize}

\noindent \textit{Note: All the modules after this point may have missing information, which will be defined for a future iteration.}

%TODO 
\subsubsection{Access Routine Semantics}
\textbf{get\_userdb\_info()}
\begin{itemize}
  \item Input: name, birthday(optional)
  \item Transition: NA
  \item Output: $\exists$ $n \in db.Name$ $\cup$ $(\exists$ $n \in db.Name$ $\cap$ $b \in db.birthday)$ $\implies$ $patient \in db$ $|$ $patient.Name = n \cap patient.birthday = b$
\end{itemize}

\textbf{PTaccount\_delete}
\begin{itemize}
...
This module allows manipulation of a patient's exercise instructions data. Despite being a smaller module,

...
\item[\texttt{export const checkEmailExists}] 
    Async function constant that checks if an email is already registered in Firebase Authentication.

\item[\texttt{export const licenseFormatValid}] 
    Function constant that validates physiotherapist license number format (XX-123456).
...
\textbf{\textit{\underline{create-physio.tsx}, \underline{create-patient.tsx}:}}
\begin{description}[style=nextline, leftmargin=2.5cm]
\item[\texttt{export const styles = StyleSheet.create(\{ ... \});}] 
    Style object containing component styling:
    \begin{itemize}
        \item \texttt{title}: Main heading style (fontSize 20, fontWeight 800, textAlign center, marginBottom 8)
        \item \texttt{label}: Input label style (fontSize 14, color \#333, marginTop 12, marginBottom 6)
        \item \texttt{input}: Text input field style (borderWidth 1, borderColor \#ddd, borderRadius 12, padding 12)
        \item \texttt{passwordHint}: Helper text style (fontSize 12, color \#666, marginTop 4, marginBottom 8, fontStyle italic)
    \end{itemize}
\end{description}
...
% validatePassword
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{validatePassword:} Validates password meets security requirements (minimum 6 characters, at least one number).} \\
\hline
\rowcolor{white}
\texttt{password: string} - User's password input &
\texttt{string | null} - Returns error message if invalid, null if valid &
None \\
\hline

% validateBirthday
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{validateBirthday:} Validates birthday matches required YYYY-MM-DD format.} \\
\hline
\rowcolor{white}
\texttt{birthday: string} - User's birthday input &
\texttt{string | null} - Returns error message if invalid, null if valid &
None \\
\hline

% onPress handler
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{onPress:} Async handler for account creation form submission. Performs validation, checks email existence, and registers patient.} \\
\hline
\rowcolor{white}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.8cm}@{}}
- Validates password via \texttt{validatePassword} \\
- Validates birthday via \texttt{validateBirthday} \\
- Checks email via \texttt{checkEmailExists} \\
- Calls \texttt{registerPatient} with: \\
  \quad \texttt{name, email, password,} \\
  \quad \texttt{inviteCode, birthday}
\end{tabular} &
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.8cm}@{}}
- Returns error message if invalid \\
- Returns null if valid \\
- \texttt{Promise<void>} on success \\
- Navigates to \texttt{"/(tabs)"} using \\
  \texttt{router.replace()}
\end{tabular} &
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.8cm}@{}}
\texttt{Error} - Catches errors and \\
displays Alert with: \\
• Password validation errors \\
• Birthday validation errors \\
...
% checkEmailExists
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{checkEmailExists:} Checks if an email is already registered in Firebase Authentication.} \\
\hline
\rowcolor{white}
...
% licenseFormatValid
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{licenseFormatValid:} Validates physiotherapist license number format (XX-123456).} \\
\hline
\rowcolor{white}
...
% registerPhysio
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{registerPhysio:} Registers a new physiotherapist account after license validation.} \\
\hline
\rowcolor{white}
...
% registerPatient
\multicolumn{3}{|>{\columncolor{lightblue}}p{13.5cm}|}{\textbf{registerPatient:} Registers a new patient account using an invite code.} \\
\hline
\rowcolor{white}

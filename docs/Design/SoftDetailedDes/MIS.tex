...
and a UI layer (\textit{exercises.tsx}). The data layer provides database operations using the \texttt{userExercises} table.
...
\end{itemize}
The exported type of this module is:
\begin{itemize}
    \item Exercise\{id,
  title,
  subtitle?,
...
\underline{\textbf{\textit{both files}}}

No exported constants are present in any of the reviewed files (exerciseData.ts, exercises.tsx). \\
...
\subsubsection{Access Routine Semantics - Local}
Since these are UI related, they are elaborated on in section \ref{sec:ui}

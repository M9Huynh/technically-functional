...
\end{description}
For additional exported constants, see \textbf{\textit{authService.ts}} exported constants in \ref{sec:validexp}. \\
For interface elements, see \ref{sec:ui}.\\
\newline
\noindent \textbf{Exported Access Programs} \\
See Table \ref{sec: accountexpprogs}, \textit{\textbf{registerPatient}} and \textit{\textbf{registerPhysio}}
...
\noindent \textbf{Environment Variables}\\
\textit{Not applicable}\\


\subsubsection{registerPatient: Internal Validation Steps}
...
\item \textbf{Physio Link Check}: If \texttt{invite.physioId = null} or does not exist, throw "Invite code missing physio link."

\item \textbf{Extract physioId}: Store \texttt{invite.physioId} for use in patient document.
\end{enumerate}

Only after all validations pass does the function proceed to:
\begin{itemize}
\item Create Firebase Auth user
\item Create Firestore user document with \texttt{role: "patient"} and \texttt{physioId}

...
This module's role is to generate real-time feedback based on form analysis on exercise that is currently being performed.
...
\item[\texttt{useEffect}] 
    React hook from "react". Used to trigger animation side effects when feedback type changes.

\item[\texttt{useRef}] 
    React hook from "react". Used to maintain a reference to the active animation loop.

\item[\texttt{Animated}] 
    React Native API from "react-native". Used to create animation values for feedback banner pulsing.

\item[\texttt{StyleSheet}] 
    React Native API from "react-native". Used to define visual styling constants for feedback components.
\end{description}

\noindent \underline{\textbf{\textit{FeedbackOverlay.ts - Variables}}}
\begin{description}
\item[\texttt{pulseAnim}] 
    Animated value used to control scaling animation timing for warning and error feedback states.

\item[\texttt{loopRef}] 
    Reference to the active animation loop, used to stop previous animations when feedback type changes.
...
\item[\texttt{FeedbackOverlayProps}] 
    Type definition for component props containing optional metrics object.
\end{description}

\noindent \underline{\textbf{\textit{External Libraries}}}
\begin{description}
\item[\texttt{React}] 
    Used for hooks (useEffect, useRef).

\item[\texttt{react-native}] 
    Used for Animated API and StyleSheet.
\end{description}

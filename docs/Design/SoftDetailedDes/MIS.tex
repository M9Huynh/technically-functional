...
\end{enumerate}  
\subsubsection{Uses}
All the variables used in each class are listed below:\\
\textbf{\textit{create-patient.tsx, create-physio.tsx}}\\
\textit{Variables used in both files are listed below.}
...
\item[\texttt{const [birthday, setBirthday] = useState("");}] 
    Manages the birthday input field; initializes as empty string.

  \end{description}
\textbf{Unique variable in \textit{\underline{create-patient.tsx}}:}
\begin{description}[style=nextline, leftmargin=2.5cm]
\item[\texttt{const [inviteCode, setInviteCode] = useState("");}] 
    Manages the invitation code input field; initializes as empty string.
\end{description}
...
\noindent\textsuperscript{2} \textit{'Birthday' is an exported constant for distinguishing two users that may have the same name. This will be integral
in a 'get\_userdb\_info' function.} \\
